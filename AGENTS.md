...
- SonarQube (`kubernetes/apps/sonarqube`) and Backstage (`kubernetes/apps/backstage`) run on the `worker` node and are on **`sargeant.co`** (`sonarqube.sargeant.co`, `backstage.sargeant.co`), not magmamoose.com, because magmamoose.com is on Tucows clientHold. Both back onto the shared CNPG `neondb_owner` database (a `Database` CR; no new Cluster). **Backstage ships a placeholder image** (`ghcr.io/calebsargeant/backstage:placeholder`) and ImagePullBackOffs by design until the real app image is built (`dockerfiles/backstage` → `ghcr.io/calebsargeant/backstage`, then bump the tag); all other wiring is production-ready. SonarQube needs `sonarqube-monitoring-passcode` in OCI Vault to become ready, and its embedded Elasticsearch relies on the chart's privileged `initSysctl` (vm.max_map_count).
